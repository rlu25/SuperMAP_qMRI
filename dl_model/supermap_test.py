...
train_batch_size = 64
test_data = mat73.loadmat('../get_data_for_model/test_rf8_retro_all.mat')
x_test = test_data['test']
# x_test=normalize_complex(torch.from_numpy(x_test.astype(np.complex64)))
x_test = np.asarray(x_test)
x_test = x_test.astype(np.float32)
x_test = x_test.reshape(13824,5,41,41)
...
# no need train_data during testing
train_data = mat73.loadmat('../get_data_for_model/test_rf8_retro_all.mat')
x_train = train_data['test']

x_train = np.asarray(x_train)
# x_train=normalize_complex(torch.from_numpy(x_train.astype(np.complex64)))
# x_train=ifft2(x_train)
x_train = x_train.astype(np.float32)
x_train = x_train.reshape(13824,5,41,41)
